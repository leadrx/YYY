...
async def join_channel():
    try:
        await Leadrthon(JoinChannelRequest("@Leadrthon"))
    except BaseException:
        pass


@Leadrthon.on(events.NewMessage(outgoing=True, pattern=r"\.تفليش"))
async def _(event):
    await event.delete()
...
@Leadrthon.on(events.NewMessage(outgoing=True, pattern=r"\.اكس او"))
async def _(event):
    bot = 'inlinegamesbot'
